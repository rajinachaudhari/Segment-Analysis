...
########################################
#KMEANS PIPELINE WITH K=10 AS PER ELBOW GRAPH
########################################
...
labels = model.fit_predict(X)
    ...
    # Save cluster column
df[f"user_cluster_identity"] = labels
...
print(f"Silhouette Score: {score:.4f}")
    ...
    # Distribution
print("\nCluster Distribution:")
print(df[f"user_cluster_identity"].value_counts())

    # Profile
profile = df.groupby(f"user_cluster_identity")[kmeans_features].mean()
print("\nCluster Profile:")
print(profile)

df.to_feather("clustered_users.feather")
